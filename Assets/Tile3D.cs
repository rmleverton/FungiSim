...
public class Tile3D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        switch (species)
        {
            ...
            case "bg":
                {
                    GetComponent<MeshRenderer>().enabled = false;  //material.color = new Color(0, 0, 0, 0);
                    break;
                }
            default:
                {
                    GetComponent<MeshRenderer>().enabled = false;
                    GetComponent<MeshRenderer>().material.color = Color.white; //black;//
                    break;
                }
        }
        WakeSeeker();
        ...
        if (species != "bg" && species != "dormantSeeker" && dormant == false)
        {
            ...
            if (val - seekers.Count > 0.2f)
            {
                ...
                if (idx + dir.x < tileRes && idx + dir.x >= 0 && idy + dir.y < tileRes && idy + dir.y >= 0 && idz + dir.z < tileRes && idz + dir.z >= 0)
                {
                    ...
                    List<GameObject> nDir = new List<GameObject>(gm.tiles[nIdFromVec].GetComponent<Tile3D>().neighbours);
                    var dirNeighbours = nDir.Intersect(neighbours).ToList();
                    //Debug.Log("ID: " + id + " dirNeighbours Length: " + dirNeighbours.Count);

                    dirNeighbours = dirNeighbours.OrderByDescending(c => c.GetComponent<Tile3D>().val).ToList();

                    for (int i = 0; i < dirNeighbours.Count; i++)
                    {
                        Tile3D nTile = dirNeighbours[i].GetComponent<Tile3D>();
                        ...
                        else if (nTile.species == "bg")
                        {
                            ...
                            else if (species == "seeker")
                            {
                                ...
                                gm.tiles[parentNodeId].GetComponent<Tile3D>().seekers.Add(dirNeighbours[i]);
                            }
                            ...
                            nTile.dir = new Vector3(nTile.idx - idx, nTile.idy - idy, nTile.idz - idz);
                            nTile.colour = colour;
    ...
    List<GameObject> FindChildren(int qID)
    {
        ...
        {
            Tile3D childTile = child.GetComponent<Tile3D>();
    ...
    void Develop()
    {
        if (species != "bg")
        {
            ...
            if (species == "seeker")
            {
                if (val >= nodeVal)
                {
                    if (!neighbours.Any(neighbour => neighbour.GetComponent<Tile3D>().species == "node"))
                    {
                        species = "node";
                        ...
                        {
                            Tile3D parentNode = gm.tiles[parentNodeId].GetComponent<Tile3D>();
                            Tile3D seeker = tile.GetComponent<Tile3D>();
            ...
            if (!dormant)
            {
                gm.tiles[parentId].GetComponent<Tile3D>().val += startingVal;// * 0.1f;
                val -= startingVal * 0.75f;// 0.9f;
            }
